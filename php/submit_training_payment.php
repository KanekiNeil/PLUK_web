...
$existingCheckCh = curl_init($supabaseUrl . '/rest/v1/training_payment?select=id&applicant_id=eq.' . urlencode($appId) . '&limit=1');
...
if ($existingCheckResponse === false || !empty($existingCheckCurlError)) {
    ...
    echo json_encode([
        'success' => false,
        'message' => 'Failed to check existing training payment.'
    ]);
    exit;
}
...
if ($existingCheckHttpCode >= 200 && $existingCheckHttpCode < 300) {
    ...
    if (is_array($existingRows) && !empty($existingRows)) {
        echo json_encode([
            ...
            'message' => 'Training payment confirmation is already submitted and pending verification.'
        ]);
        exit;
    }
}
...
[$statusUpdated, $statusUpdateError] = updateApplicantStatusWithFallback(
    ...
    $supabaseKey,
    $appId,
    'verify training payment'
);

if (!$statusUpdated) {
    echo json_encode([
        'success' => true,
        'message' => 'Training payment confirmation submitted, but applicant status was not updated.',
